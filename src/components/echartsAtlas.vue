  ...
  methods: {
    ...
    nodesHandle(params) {
      if (params.dataType == "node") {
        ...
        if (params.data.isFold) {
          ...
          this.seriesData = this.removeNodes(
            this.atlasData.data,
            globalSeriesData
          );
        ...
        this.seriesLinks.length > 0
          ? (option.series[0].links = this.seriesLinks)
          : "";
        atlas.clear();
        atlas.setOption(option);
      }
    ...
    flodNodes(nodesData, pNodeName) {
      nodesData.forEach(item => {
        ...
        if (
          ...
          parentNodes[0] == pNodeName &&
          item.name !== pNodeName
        ) {
          if (
            parentNodes[0] !== item.name &&
            !globalSeriesData.includes(item)
          ) {
          ...
          if (parentNodes.length > 1 && parentNodes.includes(pNodeName)) {
            for (let j in parentNodes) {
              if (parentNodes[j] == pNodeName) {
                item.isRelates = true;
                globalSeriesData.includes(item) == false
                  ? globalSeriesData.push(item)
                  : "";
                let relat = this.relationshipHandle(
                  pNodeName,
                  item.name,
                  "query"
                );
                relation.includes(relat) == false ? relation.push(relat) : "";
                this.seriesLinks = this.relationshipHandle(
                  pNodeName,
                  item.name,
                  "del"
                );
    ...
    openNodes(name) {
      globalSeriesData.forEach(item => {
        if (item.pNode.includes(name)) {
          this.atlasData.data.includes(item) == false
            ? this.atlasData.data.push(item)
            : "";
    ...
    removeNodes(arr, data) {
      ...
      data.forEach((item, index) => {
        if (index == 0)
          newArr = arr.filter(
            val => val.name !== item.name || item.isRelates == true
          );
        else
