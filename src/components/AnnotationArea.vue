<template>
  <div class="notes">
    ...
    <div class="note_content">
      ...
      <div class="links" v-for="(item, index) in linksList" :key="item.id">
        <div class="links_title" @click="toLink(index)">{{ item.title }}</div>
        <div class="links_website">{{ item.website }}</div>
  ...
  data() {
    return {
      notesInfo: {
        title: "持卡人",
        content:
          "所谓持卡人，就是办理卡片时所用证件的主人，比如张三用其身份证在甲银行柜台办理一张银行卡，则这张银行卡的持卡人就是张三，即使这张银行卡由李四拿着，那持卡人仍是张三，不会变成李四。该银行卡的业务办理，均视为张三办理，由此带来的相关义务也由张三承担。"
      },
      linksList: [
        {
          id: 0,
          ...
          content:
            "持卡人应即时办理挂失。信用卡挂失后的盗用风险由银行承担，挂失前48小时内的非凭密码消费对河涸海干"
        },
        {
          id: 1,
          title: "信用卡丢了怎么补办？",
